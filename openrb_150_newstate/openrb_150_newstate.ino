...
// Predefined states  
int GRIP_OPEN[2]= {530, 490}; //{450,574};   574, 470
int GRIP_CLOSE[2]= {435, 575}; //{590, 442};

//int GRIP_OPEN_NAR = {129, 157}; 
...
enum States {GRIPPER_OPENING, GRIPPER_INTER, GRIPPER_CLOSING, OBJECT_GRASPED, OBJECT_SLIPPED, GRIPPER_CLOSED, GRIPPER_OPEN, GRIPPER_RELEASE, GRIPPER_RELEASED};
enum ChangeState {CHANGE_STATE_OPEN, CHANGE_STATE_INTER, CHANGE_STATE_CLOSE, CHANGE_STATE_SAME, CHANGE_STATE_POWER, CHANGE_STATE_RELEASE};
...
{ 
  ...
  // check if velocity command is given
  // if (vel_command != 500){
  //   if (vel_command < 1023 && vel_command > 75){
  //     set_velocity(int(vel_command));
  //   }
  // }
  ...
  switch(state)
  {
    ...
    // Print "releasing" for 5 seconds
//    for (int i = 1; i < 5; i++) {
//        DEBUG_SERIAL.println("releasing");
...
void send_feedback()
{
  ...
  else if(state==GRIPPER_RELEASE) feedback_message["state"] = "GRIPPER_RELEASE";
  else if(state==GRIPPER_RELEASED) feedback_message["state"] = "GRIPPER_RELEASED";
